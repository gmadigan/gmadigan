## 👋🏻 Welcome!

### ⏱ Quick about me

* 👨🏻‍💻 AI Research Engineer at Entefy - designing, building, and deploying AI agents and ML models for enterprise clients
* 👨🏻‍🎓 PhD in physics - 5 years at CERN searching for new particles with the LHC (world's highest energy particle accelerator)
* 🐍 Python expertise - 10+ years of developing data pipelines and performing statistical analysis on TB of data
* 📖 Published author - led a search for exotic new physics with the CMS collaboration at CERN's LHC, published findings in a [peer-reviewed paper](https://journals.aps.org/prd/pdf/10.1103/PhysRevD.109.112003)
* 🇺🇸 CA Bay Area native - currently living on the US East Coast

### 🤖 What I am building

Here is my [portfolio](https://gmadigan.github.io/) showcasing past and current projects. I have hands-on development experience with:

* Generative AI: Agentic (A2A, MCP), RAG, LLMs
* Deep learning: NNs, RNNs, LSTM, CNNs, encoder-decoders
* Traditional ML: NLP, SVMs, Clustering (KMeans, KNN), Boosted Decision Trees (gradient boost, XGBoost), linear models (classification, regression)
* Fine-tuning: QLoRa, grid/random search

### 🛠 My favorite tools

These are the languages, libraries, and tools I reach for when I start exploring data.

* Languages: Python, SQL, C++
* Machine Learning: LangChain, PyTorch, Hugging Face, SentenceTransformers, Unsloth, Scikit-learn, Keras, spaCy, Gensim, Ray Tune, TorchMetrics
* Data Science: Pandas, NumPy, Scipy, Imbalanced-learn, Matplotlib, Seaborn, Jupyter Notebooks
* Databases: PostgreSQL, ChromaDB

### 🤝🏻 Let's connect

My inbox is always open! Send me a message on [LinkedIn](https://www.linkedin.com/in/gabrielmadigan/).

<!--
**gmadigan/gmadigan** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
